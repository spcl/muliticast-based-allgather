...
int main(int argc, char *argv[])
{
    ...
    set_header(HEADER);
    set_benchmark_name("osu_scatter_persistent");

    options.bench = COLLECTIVE;
    options.subtype = SCATTER_P;
    ...
    if (allocate_memory_coll((void **)&recvbuf_warmup, options.max_message_size,
                             options.accel)) {
        fprintf(stderr, "Could Not Allocate Memory [rank %d]\n", rank);
        MPI_CHECK(MPI_Abort(omb_comm, EXIT_FAILURE));
    }
    ...
    for (mpi_type_itr = 0; mpi_type_itr < options.omb_dtype_itr;
        ...
        for (size = options.min_message_size; size <= options.max_message_size;
            ...
            if (0 == rank && 1 == options.omb_enable_mpi_in_place) {
                OMB_CHECK_NULL_AND_EXIT(recvbuf, "recvbug is null");
                MPI_CHECK(MPI_Scatter_init(recvbuf, num_elements,
                                           omb_curr_datatype, MPI_IN_PLACE,
                                           num_elements, omb_curr_datatype, 0,
                                           omb_comm, MPI_INFO_NULL, &request));
            } else {
                MPI_CHECK(MPI_Scatter_init(sendbuf, num_elements,
