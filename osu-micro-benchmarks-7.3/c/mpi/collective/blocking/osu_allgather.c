...
int main(int argc, char *argv[])
{
    ...
    if (options.omb_tail_lat || options.omb_dump_lat) {
        omb_lat_arr = malloc(options.iterations * sizeof(double));
        OMB_CHECK_NULL_AND_EXIT(omb_lat_arr, "Unable to allocate memory");
    }
    if (options.omb_dump_lat) {
        omb_lat_arr_all = malloc(options.iterations * sizeof(double) * numprocs);
        OMB_CHECK_NULL_AND_EXIT(omb_lat_arr_all, "Unable to allocate memory");
    }
    print_preamble(rank);
    ...
    for (mpi_type_itr = 0; mpi_type_itr < options.omb_dtype_itr;
        ...
        for (size = options.min_message_size; size <= options.max_message_size;
                ...
                if (options.validate) {
                    ...
                    for (j = 0; j < options.warmup_validation; j++) {
                        ...
                        MPI_CHECK(MPI_Allgather(
                            ...
                            num_elements, omb_curr_datatype, omb_comm));
                    }
                    MPI_CHECK(MPI_Barrier(omb_comm));
                }
            ...
            if (options.omb_dump_lat) {
                OMB_CHECK_NULL_AND_EXIT(omb_lat_arr, "Passed array is NULL");
                OMB_CHECK_NULL_AND_EXIT(omb_lat_arr_all, "Passed array is NULL");
                MPI_CHECK(MPI_Gather(omb_lat_arr, options.iterations, MPI_DOUBLE,
                                     omb_lat_arr_all, options.iterations, MPI_DOUBLE,
                                     0, omb_comm));
                if (rank == 0) {
                    dump_lat_measurements(options.omb_dump_root_dir, "allgather",
                                          omb_lat_arr_all, numprocs, -1, size, options.iterations);
                }
            }
