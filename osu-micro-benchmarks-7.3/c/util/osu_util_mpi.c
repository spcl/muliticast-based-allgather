...
void dump_lat_measurements(char *dump_root_dir, char *coll_name, double *measurements, 
                           int numprocs, int root, int size, size_t iterations)
{
    FILE *dump_file;
    char dump_file_name[OMB_MAX_DUMP_FILE_NAME_LENGTH];
    int proc_id;
    size_t iter;

    snprintf(dump_file_name, OMB_MAX_DUMP_FILE_NAME_LENGTH, 
             "%s/lat_measurements.%s.%d.%zu.%d",
             dump_root_dir, coll_name, numprocs, iterations, size);
    dump_file = fopen(dump_file_name,"w");
    OMB_CHECK_NULL_AND_EXIT(dump_file, "failed to open dump file");
    fprintf(dump_file, "msg_size;proc_id;root;iter;latency\n");
    for (proc_id = 0; proc_id < numprocs; proc_id++) {
        for (iter = 0; iter < iterations; iter++) {
            fprintf(dump_file, 
                    "%d;%d;%d;%zu;%lf\n",
                    size, proc_id, root, iter, measurements[proc_id * iterations + iter]);
        }
    }
    fclose(dump_file);
}
